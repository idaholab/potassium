...
registerMooseObject("PotassiumApp", PotassiumVaporFluidProperties);

InputParameters
PotassiumVaporFluidProperties::validParams()
{
...
PotassiumVaporFluidProperties::PotassiumVaporFluidProperties(const InputParameters & parameters)
...
Real
PotassiumVaporFluidProperties::p_from_v_e(Real v, Real e) const
{
  v *= _to_ft3_lb;
  e *= _to_Btu_lb;

  double p, T;
  int ierr = FLASH_vu_G_K(v, e, T, p);
  if (ierr != 0)
    return getNaN();
  else
    return p * _to_Pa;
...
void
PotassiumVaporFluidProperties::p_from_v_e(
    Real v, Real e, Real & p, Real & dp_dv, Real & dp_de) const
{
  ...
  else
  {
    ...
    DERIV_vu_G_K(T, p, dp_dv_u, dp_du_v, dt_dv_u, dt_du_v);
...
Real
PotassiumVaporFluidProperties::T_from_v_e(Real v, Real e) const
{
  v *= _to_ft3_lb;
  e *= _to_Btu_lb;
...
void
PotassiumVaporFluidProperties::T_from_v_e(
...
Real
PotassiumVaporFluidProperties::c_from_v_e(Real v, Real e) const
{
  v *= _to_ft3_lb;
  e *= _to_Btu_lb;
  ...
  {
    double w, dwdt, dwdp;
    DIFF_w_tp_G_K(T, p, w, dwdt, dwdp);
    return w * _to_m;
  }
}

void
PotassiumVaporFluidProperties::c_from_v_e(
...
Real
PotassiumVaporFluidProperties::cp_from_v_e(Real v, Real e) const
{
  v *= _to_ft3_lb;
  e *= _to_Btu_lb;
  ...
  else
  {
    ...
    DIFF_cp_tp_G_K(T, p, cp, dcpdt, dcpdp);
...
void
PotassiumVaporFluidProperties::cp_from_v_e(
...
Real
PotassiumVaporFluidProperties::cv_from_v_e(Real v, Real e) const
{
  v *= _to_ft3_lb;
  e *= _to_Btu_lb;
  ...
  else
  {
    ...
    DIFF_cv_tp_G_K(T, p, cv, dcvdt, dcvdp);
...
void
PotassiumVaporFluidProperties::cv_from_v_e(
...
Real
PotassiumVaporFluidProperties::mu_from_v_e(Real v, Real e) const
{
  v *= _to_ft3_lb;
  e *= _to_Btu_lb;
  ...
  else
  {
    ...
    etav_t_K(T, eta, detadt, d2etadt2); // eta in lbm/ft-hr
...
void
PotassiumVaporFluidProperties::mu_from_v_e(
    Real v, Real e, Real & mu, Real & dmu_dv, Real & dmu_de) const
{
  ...
  else
  {
    ...
    etav_t_K(T, mu, dmudt, d2mudt2); // eta in lbm/ft-hr
...
Real
PotassiumVaporFluidProperties::mu_from_p_T(Real p, Real T) const
{
  p *= _to_atm;
  T *= _to_R;
...
Real
PotassiumVaporFluidProperties::k_from_v_e(Real v, Real e) const
{
  v *= _to_ft3_lb;
  e *= _to_Btu_lb;
  ...
  else
  {
    ...
    lambdav_t_K(T, lambda, dlambdadt, d2lambdadt2); // lambda in Btu/hr-ft-F
...
void
PotassiumVaporFluidProperties::s_from_v_e(
...
Real
PotassiumVaporFluidProperties::s_from_h_p(Real h, Real p) const
{
  p *= _to_atm;
  h *= _to_Btu_lb;

  double T;
  int ierr = FLASH_ph_G_K(p, h, T);
  if (ierr != 0)
    return getNaN();
  else
  {
...
void
PotassiumVaporFluidProperties::s_from_h_p(
    Real h, Real p, Real & s, Real & ds_dh, Real & ds_dp) const
{
  ...
  else
  {
    ...
    DIFF_s_tp_G_K(T, p, s, dsdt, d2sdt2, dsdp, d2sdp2, d2sdtdp);
    DIFF_h_tp_G_K(T, p, h_, dhdt, d2hdt2, dhdp, d2hdp2, d2hdtdp);
...
Real
PotassiumVaporFluidProperties::rho_from_p_s(Real p, Real s) const
{
  p *= _to_atm;
  s *= _to_Btu_lbR;

  double T;
  int ierr = FLASH_ps_G_K(p, s, T);
  if (ierr != 0)
    return getNaN();
  else
  {
    double v, dvdt, d2vdt2, dvdp, d2vdp2, d2vvdtdp;
    DIFF_v_tp_G_K(T, p, v, dvdt, d2vdt2, dvdp, d2vdp2, d2vvdtdp);
...
void
PotassiumVaporFluidProperties::rho_from_p_s(
    Real p, Real s, Real & rho, Real & drho_dp, Real & drho_ds) const
{
  ...
  else
  {
    ...
    DIFF_v_tp_G_K(T, p, v, dvdt, d2vdt2, dvdp, d2vdp2, d2vdtdp);
    DIFF_s_tp_G_K(T, p, s_, dsdt, d2sdt2, dsdp, d2sdp2, d2sdtdp);
...
Real
PotassiumVaporFluidProperties::e_from_v_h(Real v, Real h) const
{
  v *= _to_ft3_lb;
  h *= _to_Btu_lb;
  ...
  else
  {
    ...
    double h_, dhdt, d2hdt2, dhdp, d2hdp2, d2hdtdp;
    double u;
    DIFF_v_tp_G_K(T, p, v_, dvdt, d2vdt2, dvdp, d2vdp2, d2vdtdp);
    DIFF_h_tp_G_K(T, p, h_, dhdt, d2hdt2, dhdp, d2hdp2, d2hdtdp);
...
void
PotassiumVaporFluidProperties::e_from_v_h(
...
Real
PotassiumVaporFluidProperties::rho_from_p_T(Real p, Real T) const
{
  p *= _to_atm;
  T *= _to_R;
...
void
PotassiumVaporFluidProperties::rho_from_p_T(
...
Real
PotassiumVaporFluidProperties::e_from_p_rho(Real p, Real rho) const
{
  p *= _to_atm;
  rho /= _to_ft3_lb;
  double v = 1. / rho;
  ...
  double T;
  int ierr = FLASH_prho_G_K(p, rho, T);
  if (ierr != 0)
    return getNaN();
  else
  {
...
void
PotassiumVaporFluidProperties::e_from_p_rho(
...
Real
PotassiumVaporFluidProperties::h_from_p_T(Real p, Real T) const
{
  p *= _to_atm;
  T *= _to_R;
...
void
PotassiumVaporFluidProperties::h_from_p_T(
...
Real
PotassiumVaporFluidProperties::s_from_p_T(Real p, Real T) const
{
  p *= _to_atm;
  T *= _to_R;
...
void
PotassiumVaporFluidProperties::s_from_p_T(
...
Real
PotassiumVaporFluidProperties::p_from_h_s(Real h, Real s) const
{
  h *= _to_Btu_lb;
  s *= _to_Btu_lbR;

  double T, p;
  int ierr = FLASH_hs_G_K(h, s, T, p);
  if (ierr != 0)
    return getNaN();
  else
    return p * _to_Pa;
...
void
PotassiumVaporFluidProperties::p_from_h_s(
...
Real
PotassiumVaporFluidProperties::g_from_v_e(Real v, Real e) const
{
  v *= _to_ft3_lb;
  e *= _to_Btu_lb;
...
Real
PotassiumVaporFluidProperties::beta_from_p_T(Real p, Real T) const
{
...
void
PotassiumVaporFluidProperties::beta_from_p_T(
...
Real
PotassiumVaporFluidProperties::criticalDensity() const
{
  return 192.;
}
